...
namespace TarsierSpaceTech
{
    ...
    internal class TST_KSPAddonImproved : Attribute
    {
        [Flags]
        public enum Startup
        {
        ...
        public TST_KSPAddonImproved(Startup mask, bool once = false)
        {
            runOnce = once;
            scenes = mask;
        }
    ...
    internal class CustomAddonLoader : MonoBehaviour
    {
        ...
        class AddonInfo
        {
            ...
            public readonly TST_KSPAddonImproved addon;
            public bool created;

            internal AddonInfo(Type t, TST_KSPAddonImproved add)
            {
                type = t;
                created = false;
            ...
            internal TST_KSPAddonImproved.Startup Scenes
            {
                get
                {
                    return addon.scenes;
        ...
        void Awake()
        {
            ...
            {
                var attr = ((TST_KSPAddonImproved[])ourType.GetCustomAttributes(typeof(TST_KSPAddonImproved), true)).SingleOrDefault();
                if (attr != null)
                {
        ...
        void OnLevelWasLoaded(int level)
        {
            ...
            TST_KSPAddonImproved.Startup mask = 0;
            ...
            switch (scene)
            {
                case GameScenes.EDITOR:
                    mask = TST_KSPAddonImproved.Startup.Editor;
                    break;

                case GameScenes.CREDITS:
                    mask = TST_KSPAddonImproved.Startup.Credits;
                    break;

                case GameScenes.FLIGHT:
                    mask = TST_KSPAddonImproved.Startup.Flight;
                    break;

                case GameScenes.LOADING:
                    mask = TST_KSPAddonImproved.Startup.Instantly;
                    break;

                case GameScenes.MAINMENU:
                    mask = TST_KSPAddonImproved.Startup.MainMenu;
                    break;

                case GameScenes.SETTINGS:
                    mask = TST_KSPAddonImproved.Startup.Settings;
                    break;

                case GameScenes.SPACECENTER:
                    mask = TST_KSPAddonImproved.Startup.SpaceCenter;
                    break;

                case GameScenes.TRACKSTATION:
                    mask = TST_KSPAddonImproved.Startup.TrackingStation;
                    break;

                case GameScenes.PSYSTEM:
                    mask = TST_KSPAddonImproved.Startup.PSystemSpawn;
                    break;
